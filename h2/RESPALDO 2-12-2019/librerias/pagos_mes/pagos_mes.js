...
function ruta_imprimir_pdf(vlc_proveedor,vlc_fecha,vlc_transaccion)
{
	window.location=('http://app.hmetro.med.ec/honorarios/controlador/descarga_documentos/preparar_pdf.php?proveedor='+vlc_proveedor+'&fecha='+ vlc_fecha +'&transaccion='+vlc_transaccion+'');
}	

function lista_pagos_mes(vln_proveedor,vln_anio,vln_mes,vlc_tipo_busqueda)
	{
		...
		$.ajax(
		{
			url:'http://app.hmetro.med.ec/honorarios/controlador/pagos_mes/pagos_mes.php',
			type:'POST',
			data:'proveedor='+vln_proveedor+'&boton=buscar'+'&anio='+vln_anio+'&mes='+vln_mes+'&tipo_busqueda='+vlc_tipo_busqueda+'&forma_busqueda='+vlc_forma_busqueda+'&paciente='+vlc_paciente+'&factura='+vln_factura,
			beforeSend: function(){
				$('#fade').fadeIn();
				$('#light').fadeIn();
			}	
		...
		{	 
			...
			if(vlc_valores==''){
				...
				{					
					...
					html+="<tr><td align='center'>"+vlc_valores[i][0]+"</td><td>"+vlc_valores[i][1]+"</td><td>"+vlc_valores[i][2]+"</td><td>"+vlc_valores[i][3]+"</td><td>"+vlc_valores[i][4]+"</td><td>"+vlc_valores[i][6]+"</td><td>"+vlc_valores[i][7]+"</td><td id='tabla_pagos_mes_monto'>"+vlc_valores[i][5]+"</td><td><button class='btn btn-default btn-xs' data-toggle='modal' data-target='#ver_detalle' onclick='lista_factura_detalle("+'"'+vlc_valores[i][8]+'"'+","+'"'+vlc_valores[i][0]+'"'+","+'"'+vlc_valores[i][4]+'"'+");factura_detalle_total("+'"'+vlc_valores[i][8]+'"'+","+'"'+vlc_valores[i][0]+'"'+","+'"'+vlc_valores[i][4]+'"'+");'><span data-toggle='tooltip' title='Ver detale de Pago' class='glyphicon glyphicon-eye-open'></span></button><button class='btn btn-default btn-xs' id='btn_descargar_pdf' title='Descargar PDF'  onclick=' ruta_imprimir_pdf("+'"'+vlc_valores[i][8]+'"'+","+'"'+vlc_valores[i][0]+'"'+","+'"'+vlc_valores[i][4]+'"'+")'><span class='glyphicon glyphicon-save'></span></button></td></tr> ";
				}				
...
function imprimir_pdf(vlc_proveedor,vlc_fecha,vlc_transaccion)
	{
		$.ajax(
		{
			url:'http://app.hmetro.med.ec/honorarios/controlador/descarga_documentos/preparar_pdf.php',
			type:'POST',
			data: 'proveedor='+vlc_proveedor+'&boton=ver'+'&fecha='+vlc_fecha+'&transaccion='+vlc_transaccion	
...
$("#boton_buscar").click(function()
	{
		...
		lista_pagos_mes(vln_proveedor_select,vln_anio_select,vln_mes_select,'simple');
	});
