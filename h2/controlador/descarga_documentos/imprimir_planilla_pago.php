<?php
	...
	require_once("../../controlador/master/master.php");
		...
		$vgc_tipo_imprime = $_GET["vgc_tipo_imprime"];
		$vgc_instancia_planillas = new planillas_pago();

		if ($vgc_tipo_imprime =="DESCUENTOS") 
		{
			$vgc_array_transaccion=$vgc_instancia_planillas ->validar_transaccion_descuento($vgc_proveedor,$vgn_numero_transaccion);
			$vgc_tipo_transaccion= $vgc_array_transaccion[0][0];

			$vgc_array_numero_referencia=$vgc_instancia_planillas ->consultar_numero_referencia_descuento($vgc_proveedor,$vgn_numero_transaccion);
		...
		if ($vgc_tipo_transaccion =="PH") 
		{
			...
			$vlc_funcion_nombre = "PAGOS_PROCESADOS_PLANILLA_DESCARGA";
			$vgc_instancia_planillas-> guardar_funcion_auditoria($vgc_proveedor, $vlc_funcion_nombre, $vlc_ip);
			...
			$vlc_ip=gethostbyaddr($_SERVER['REMOTE_ADDR']);
			$vlc_funcion_nombre = "PAGOS_PROCESADOS_COMPROBANTE_DESCARGA";
			$vgc_instancia_planillas-> guardar_funcion_auditoria($vgc_proveedor, $vlc_funcion_nombre, $vlc_ip);
