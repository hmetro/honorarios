<?php  
	class honorarios
	{
		...
		function lista_pagos_paciente($vln_proveedor,$vlc_paciente)
		{
			...
			$str = str_replace(' ', '%', $vlc_paciente);

			$vlc_paciente = strtoupper($str);
      ...
      c.fecha,  --0
            ...
            (SELECT t1.pk_nhcl hist_clinica,DECODE (discriminante, 'NTL', primer_apellido || ' '
    || segundo_apellido || ' ' || primer_nombre || ' ' || segundo_nombre, 
           razon_social) nombre
    FROM   cad_pacientes t1, bab_personas t2
    WHERE  t1.fk_persona = t2.pk_codigo)y
            where a.no_cia = '01'
            and nvl(c.anulado,'N') = 'N'
            and md.hist_clinica_pac = y.hist_clinica (+)
            and y.nombre like '%$vlc_paciente%'
            and a.no_prove = '$vln_proveedor'
            and c.tipo_doc in ('CK','TR')
            and a.no_cia = b.no_cia
            and a.tipo_doc = b.tipo_doc
